  ...
  module Querying
    ...
    module ClassMethods
      ...
      def find_by_esql(esql, allow_partial_results: nil, &block)
        result = with_connection do |c|
          _query_by_sql(c, _esql_query(c, esql, allow_partial_results))
        end
      ...
      def esql(esql, allow_partial_results: nil)
        with_connection do |c|
          c.exec_query(_esql_query(c, esql, allow_partial_results), "#{name} ES|QL")
        end
      end
      ...
      def _esql_query(connection, esql, allow_partial_results = nil)
