...
module Arel # :nodoc: all
  module Visitors
    module ElasticsearchQuery
      ...
      #
      # PLEASE NOTE: +Arel::Nodes::Or+ is a +Nary+ node - it may carry MORE than two children
      # (+ActiveRecord::Relation::WhereClause#or+ already appends to an existing Or).
      # Resolving it through +left+ / +right+ would silently drop every further operand.
      # @param [Object] node
      # @return [Array]
      def _flatten_or(node)
        return [node] unless node.is_a?(::Arel::Nodes::Or)

        node.children.flat_map { |child| _flatten_or(child) }
      end
