        ...
        log(gate, arguments, name, async: async, statistics: (statistics = {})) do
          with_raw_connection(allow_retry: allow_retry, materialize_transactions: materialize_transactions) do |conn|
            ...
            if response.is_a?(::Elasticsearch::API::Response)
              ...
              # raise PARTIAL responses (unless disabled).
              ...
              if response['is_partial']
                raise(::ElasticsearchRecord::PartialResultsError, gate) if ::ElasticsearchRecord.error_on_partial_results

                # accepted partial results are at least surfaced through the log subscriber
                statistics[:warnings] = Array(statistics[:warnings]) + ["Elasticsearch api request returned PARTIAL results (#{gate})"]
              end
            end
