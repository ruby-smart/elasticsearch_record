...
* [add] **range & comparison predicates** - `where(year: 2020..2021)`, `2020...2021`, `2020..`, `..2021`, `nil..nil` and `where.not(...)` now compile into a `range` query. They used to raise an `UnsupportedVisitError`, so the only way to build a range was the raw `filter(range: ...)` escape hatch. New visits: `Between`, `GreaterThan`, `GreaterThanOrEqual`, `LessThan`, `LessThanOrEqual`, `Not` & `NotIn`
  * only the modern `gte` / `gt` / `lte` / `lt` keys are generated - `from` / `to` / `include_lower` / `include_upper` are deprecated since **8.16**. This also matches the read direction of the `range` TYPE
  * an exclusive range is merged into ONE clause _(`{gte: 2020, lt: 2021}`)_. Merging is restricted to a **lower with an upper** bound of the same field - merging two bounds of the same half would keep only one of them and silently **widen** the query
  * a negated conjunction is wrapped into a nested `bool` _(De Morgan)_ - flattening it into sibling `must_not` clauses would turn `NOT(a AND b)` into `NOT a AND NOT b`
* [fix] **`#or` silently returned NOTHING** - ActiveRecord wraps an OR into an `Arel::Nodes::Grouping`, and the visitor failed EVERY grouping, which swapped in the match-nothing `FAILED_BODIES` body. It now compiles into a `bool.should` with `minimum_should_match: 1` _(elasticsearch defaults that to 0 as soon as the `bool` also carries a `filter`, so without it every `should` would degrade to a pure scoring hint)_. Also adds `visit_ElasticsearchRecord_Relation_QueryClause` for the `query_clause` side of an OR
* [fix] **`truncate_table` / `create_table(copy_from:)` / `change_table(recreate: true)` destroyed mapping nodes** - `transform_mappings!` only read `_meta` & `properties`, so `dynamic`, `dynamic_templates`, `runtime`, `_source`, `_routing`, `date_detection` & co were dropped from the definition the index was recreated from. They are now carried through verbatim _(`CreateTableDefinition#mapping_options`)_
* [fix] **`Relation#timeout` was unusable** - its `value = true` default sent `?timeout=true`, which elasticsearch rejects with a `400 illegal_argument_exception`. The argument is now required and validated as an elasticsearch time value up front. `nil` / `false` removes it again
* [fix] **`Result#total` reported a capped count as exact** - elasticsearch stops counting at 10.000 hits and answers `{value: 10000, relation: 'gte'}`. Adds `#total_relation` & `#total_exact?` _(also on the relation)_ to tell a real total apart from a lower bound. `total` no longer raises for a `track_total_hits: false` query _(it returns `nil`)_, and resolves a plain Integer total _(`rest_total_hits_as_int`)_
* [add] **deprecation warnings are surfaced** - elasticsearch announces every syntax it is going to remove through a HTTP `Warning` header. Those are read in `#api`, published as `payload[:statistics][:warnings]` and logged by the `LogSubscriber` _(at WARN, independent of the log level, each distinct message once per process)_
* [add] **ES|QL `is_partial`** - since 8.19 a ES|QL query answers with PARTIAL results instead of failing. Adds `Result#is_partial?`, `#documents_found` & `#values_loaded`, the `ElasticsearchRecord.error_on_partial_results` flag _(default: `false`, warns instead)_ and an `allow_partial_results:` argument on `.esql` / `.find_by_esql`. Also adds the runtime guard for the documented `>= 8.11` requirement
* [add] `Relation#knn` for approximate nearest neighbour search - a regular part of the search body since **8.12**, so it combines with the `query` / `filter` chain
* [add] `Relation#restrict` to filter the transferred `_source` _(`includes:`, `excludes:` and the 8.19 `exclude_vectors:`)_ - the method was listed in the README but had never existed
* [add] index name validation in `create_table` - fails with a readable `ArgumentError` instead of an `invalid_index_name_exception` that names the **resolved** _(prefixed & suffixed)_ index
* [fix] `point_in_time` leaked its PIT whenever the block raised - the close is now also run on the error path _(and a failing close no longer replaces the original exception)_. The `pit_results` cursor reads `sort` / `pit_id` defensively
* [ref] gemspec locks `elasticsearch` to `>= 8.0, < 9` _(was `>= 7.17`)_ - a 7.x client cannot address an 8.x server outside the compatibility mode, and the 9.x client sends a `compatible-with=9` header that 8.x servers reject
* [ref] removed `_type` from `METADATA_FIELDS` - elasticsearch dropped mapping types with 7.0 and stopped returning the field with 8.0, so it only ever resolved to `nil`
* [ref] `TableMappingDefinition::ATTRIBUTES` & `TableSettingDefinition::DYNAMIC_NAMES` extended for the 8.x types and settings _(`dims`, `element_type`, `inference_id`, `metrics`, `value`, `time_series_*`, `lifecycle.*`, `mapping.*` incl. the 8.15+ `mapping.source.mode`, ...)_ - they are only enforced under `strict: true`, which was therefore unusable with `dense_vector`, `semantic_text` & `aggregate_metric_double`
* [ref] `cluster_settings` reads `persistent` & `transient` defensively
...
* [add] specs for `ElasticsearchRecord::Persistence`, `ModelApi`, `SchemaMigration`, `Querying::ClassMethods`, the `Relation` methods _(Core, Query, Calculation & Result)_, the `Elasticsearch::SchemaStatements`, `TableStatements` & `SchemaDumper`, every class of the `Elasticsearch::Type` namespace _(incl. its `TYPE_MAP` registration)_, `Arel::Collectors::ElasticsearchQuery` and all three `Arel::Visitors::Elasticsearch*` visitors - incl. the 1.8.1 "only ten migrations" and 1.8.2 nested-reset regressions and a `TestIndexWithAutoIncrement` support that builds a model against a real `auto_increment` schema _(destructive calls still run through `TestIndex` and its name guard)_. Behaviours pinned as-is:
  * AR's blank-arg stripping silently drops `configure(key, nil)` _(only the Hash form removes a key)_
