...
## [Unreleased]
* [ref] **BREAKING**: every table statement of `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements` now resolves its table (index) name(s) through `#_env_table_name` **by default** - the `table_name_prefix` / `table_name_suffix` of the connection config no longer have to be applied by hand. Forgetting the former, opt-in `_env_table_name(...)` call silently wrote into the **undecorated** index, which on a shared cluster means the index of another environment - the default now fails safe. A new `decorate:`-argument _(default: `true`)_ switches it off per call, for names that are already resolved and for base names that start with the prefix _(or end with the suffix)_, which `#_env_table_name` cannot tell apart from a resolved one. Affects `open_table(s)`, `close_table(s)`, `refresh_table(s)`, `truncate_table(s)`, `drop_table`, `block_table`, `unblock_table`, `clone_table`, `backup_table`, `restore_table`, `rename_table`, `reindex_table`, `create_table` _(incl. `copy_from`)_, `change_table` and every mapping / meta / setting / alias statement _(which forward it through `#_exec_change_table_with`)_. Only TABLE names are decorated - `alias`, `mapping`, `setting` & `meta` names are never touched, and the **schema** statements stay undecorated, since ActiveRecord & the schema dumper call them with an already resolved name. Existing `_env_table_name(...)` calls keep working _(the method is idempotent and stays public)_
* [add] `ElasticsearchRecord.decorate_table_names` _(default: `true`)_ as global kill-switch for the table name decoration - setting it to `false` restores the former, opt-in behaviour, where the `table_name_prefix` / `table_name_suffix` had to be applied by hand through `#_env_table_name`. It only provides the DEFAULT for a **not explicitly provided** `decorate:`-argument, so a single statement can still opt in or out on its own _(`decorate: true` keeps working while the flag is off)_. The `#_env_table_name`-method itself is the raw resolver and stays unaffected
* [ref] `ActiveRecord::ConnectionAdapters::Elasticsearch::SchemaDumper#table` no longer wraps the dumped name into a `_env_table_name(...)` call - the statement decorates it on its own, so the **base** name is dumped instead. If that base name does not resolve back to the real index _(a base name colliding with the prefix / suffix, dumper options differing from the connection config, or a globally disabled decoration)_, the full name is dumped with an explicit `decorate: false` - so a dumped schema stays correct even if `ElasticsearchRecord.decorate_table_names` is flipped before it is loaded
* [ref] **BREAKING**: `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements#open_tables`, `#close_tables`, `#refresh_tables` & `#truncate_tables` no longer subtract the AR-internal indices _(`schema_migrations` & `ar_internal_metadata`)_ from the provided names. An explicitly named index was silently dropped, although `refresh` is harmless and `open` is the recovery path for a closed `schema_migrations` - the four statements are plain loops now and forward the raw `decorate:`-flag to their singular statement, which owns both the resolution and the guard. They return an `Array` for every provided name instead of a `nil` for an emptied list
* [add] `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements#truncate_table` now raises an `ArgumentError` for an AR-internal index - elasticsearch has no `truncate` concept, so the statement runs a `drop` & `create` and would wipe the migration state of the whole environment. The check runs on the **already resolved** name and `#_internal_table_names` provides both the resolved and the decorated form, so neither a base nor a resolved name slips through _(`schema_migration` resolves through `ElasticsearchRecord::Base` while `internal_metadata` resolves through `ActiveRecord::Base`, so the two do not have to carry the same prefix & suffix)_. `#drop_table` deliberately stays **unguarded**: `ActiveRecord::SchemaMigration#drop_table` & `ActiveRecord::InternalMetadata#drop_table` reset both tables through exactly that statement
* [fix] `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements#rename_table` to resolve both table names **itself** instead of only forwarding the `decorate:`-flag to the statements it calls - the `schema_cache.clear_data_source_cache!` and the `cluster_health` call in between address the index directly and were left with the undecorated name, so on a prefixed / suffixed connection the wrong cache entry was cleared and the health check waited for an index that does not exist _(until the read timeout)_
* [fix] `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements#backup_table` to build the auto-generated target from the **already resolved** name - it would otherwise append the suffix behind the `-snapshot-` part
* [fix] `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements#_env_table_name` to cast the prefix & suffix - `table_name_prefix:` without a value is a valid yml entry and resolves to `nil`, which raised a `TypeError` in the `start_with?` check _(harmless while the method was opt-in, but it now runs on every table statement)_
* [ref] `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements` drops the stale `:rename_table` entry from its `define_unsupported_method` list - the real implementation was defined below it and overwrote the raising one anyway
...
* [fix] `ElasticsearchRecord::Persistence::ClassMethods#_insert_with_auto_increment` to write the **plain, integer** id into the `_meta.auto_increment`. The yielded block does NOT resolve a plain id: it runs through `connection.insert`, which returns an **Array** of the `returning` column values _(ActiveRecord always provides the primary_key as returning column, see `ActiveRecord::Persistence#_create_record`)_ - and that Array was stored as-is, so the NEXT insert into an `auto_increment` table raised a `NoMethodError` on `Array#to_i`. An `auto_increment` index could therefore only ever take a **single** record. unwraps an `Array`, a `Hash` _(`{id: ...}`, the shape of the yielded arguments)_ or a plain value, and the resolved id is cast to an `Integer` - elasticsearch resolves a document `_id` as String, which would otherwise flip the type of the meta behind the first insert. The block result itself is returned unchanged, since ActiveRecord zips it against the returning columns
* [add] specs for `ElasticsearchRecord::Persistence` _(covers `_insert_record`, `_update_record`, `_delete_record`, the private `_insert_with_auto_increment` against a real `auto_increment` schema - meta resolution, the primary-key maximum fallback, a manually provided id, every block-result shape and the end-to-end `create` write-back - and the instance-level `_create_record`)_ - pins that `_update_record` / `_delete_record` always return `0` _(a document-API response carries no `total`)_, that every column is a pass-through `MulticastValue` and that an auto-incremented id is only written as document `_id`, never into the mapped primary-key field
* [add] `TestIndexWithAutoIncrement` spec support _(creates & drops the test index with an `auto_increment` primary key, exposes its current `_meta` and builds a model against it)_ - destructive calls still run through `TestIndex`, which owns the index-name guard
* [add] specs for `ActiveRecord::ConnectionAdapters::Elasticsearch::SchemaDumper` _(covers the expanded prefix / suffix options, `ignored?` / `ignored_table?`, `format_attribute`, the dumped `create_table` block incl. the `nested_blocks` variant, the name decoration _(base name vs. `decorate: false`)_, the per-table rescue and `dump`)_ - pins that an index **alias** is dumped as a mapping of the elasticsearch 'alias' field type, which makes the `t.alias` branch of the dumper unreachable
* [add] specs for `ActiveRecord::ConnectionAdapters::Elasticsearch::TableStatements` _(covers `backup_table`, `restore_table`, `rename_table`, `reindex_table`, `clone_table`, `unblock_table`, the mapping / meta / setting / alias statements incl. the `add_column` / `change_column` / `remove_column` aliases, `_env_table_name` and the `decorate:`-argument incl. an end-to-end context against a suffixed connection)_
