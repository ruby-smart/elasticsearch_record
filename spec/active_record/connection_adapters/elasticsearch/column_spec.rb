...
RSpec.describe ActiveRecord::ConnectionAdapters::Elasticsearch::Column do
  # the resolved cast type - +new_column_from_field+ wraps every type into a +MulticastValue+
  let(:cast_type) do
    ActiveRecord::ConnectionAdapters::Elasticsearch::Type::MulticastValue.new(nested_type: ActiveRecord::Type::String.new)
  end
  ...
  def build_column(name = 'code', **kwargs)
    described_class.new(name, cast_type, nil, sql_type_metadata, **kwargs)
  end
  ...
  describe '#==' do
    ...
    it 'is not equal to a base column' do
      base = ActiveRecord::ConnectionAdapters::Column.new('code', cast_type, nil, sql_type_metadata)
