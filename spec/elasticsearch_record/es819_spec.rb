...
RSpec.describe 'Elasticsearch 8.19 compatibility', :elasticsearch do
  ...
  describe ElasticsearchRecord::Result do
    ...
    it 'reports a partial result' do
      expect(described_class.new({ 'is_partial' => true }).partial?).to be(true)
      expect(described_class.new({ 'is_partial' => false }).partial?).to be(false)
      expect(described_class.new({}).partial?).to be(false)
    end
  end
  ...
  describe 'ES|QL' do
    ...
    # PLEASE NOTE: only TABULAR (ES|QL) responses are faked - the index cleanup of the +after+ hook
    # runs through the same response class.
    def fake_partial_responses!
      allow_any_instance_of(Elasticsearch::API::Response).to receive(:[]) do |resp, key|
        key == 'is_partial' && resp.body.is_a?(Hash) && resp.body.key?('columns') ? true : resp.body[key]
      end
    end
    ...
    it 'reports a partial result as warning while error_on_partial_results is disabled' do
      begin
        ...
        ActiveSupport::Notifications.subscribed(->(*, payload) {
          captured ||= payload[:statistics][:warnings] if payload[:statistics].is_a?(Hash) && payload[:gate] == 'esql.query'
        }, 'query.elasticsearch_record') do
          expect { model.esql("FROM #{TestIndex.name} | LIMIT 1") }.not_to raise_error
        end

        expect(captured).to include(a_string_matching(/PARTIAL results \(esql\.query\)/))
      ensure
        ElasticsearchRecord.error_on_partial_results = true
      end
    end
